...
THEME=github
USER_HOME := $(HOME)
DISABLE_LIVERELOAD=0
BUNDLE_USER_HOME=$(USER_HOME)/tmp
BUNDLE_PATH=$(USER_HOME)/tmp/bundle
RUBOCOP_CACHE_ROOT=/private/tmp/rubocop_cache
RUBY_BUNDLE_ENV=mkdir -p "$(BUNDLE_USER_HOME)" "$(BUNDLE_PATH)" "$(RUBOCOP_CACHE_ROOT)" && BUNDLE_USER_HOME="$(BUNDLE_USER_HOME)" BUNDLE_PATH="$(BUNDLE_PATH)" RUBOCOP_CACHE_ROOT="$(RUBOCOP_CACHE_ROOT)" SECRET_PAGE_PASSWORD="staging"
RUBY_BUNDLE=$(RUBY_BUNDLE_ENV) bundle exec
CODEX_BUNDLE_USER_HOME=/private/tmp/bundle
CODEX_RUBOCOP_CACHE_ROOT=/private/tmp/rubocop_cache
CODEX_RUBY_BUNDLE=source ~/.zshrc && mkdir -p "$(CODEX_BUNDLE_USER_HOME)" "$(BUNDLE_PATH)" "$(CODEX_RUBOCOP_CACHE_ROOT)" && DISABLE_LIVERELOAD="$(DISABLE_LIVERELOAD)" BUNDLE_USER_HOME="$(CODEX_BUNDLE_USER_HOME)" BUNDLE_PATH="$(BUNDLE_PATH)" RUBOCOP_CACHE_ROOT="$(CODEX_RUBOCOP_CACHE_ROOT)" bundle exec
...
ai:: tags
	...
	agy --dangerously-skip-permissions -i "`./scripts/check_tags.rb source/diary/*/*`"
