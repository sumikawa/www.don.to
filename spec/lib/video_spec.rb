...
RSpec.describe do
  describe 'detect' do
    ...
    it 'return ffmpeg to generate hdvideo' do
      ...
      expect(result).to eq(' -g 120 -vcodec libx264 -s 480x270 -bt 1536k -acodec aac -ar 32000 -ac 1 -ab 48k -movflags faststart  -f mp4')
    end

    it 'return ffmpeg to generate hdtrvideo' do
      result = Video.cmd_opts(opts: Video.detect(sample2), acodec: 'aac', vcodec: 'mp4')
      expect(result).to eq('-metadata:s:v:0 rotate=0 -g 120 -vcodec libx264 -s 270x480 -bt 1536k -acodec aac -ar 32000 -ac 1 -ab 48k -movflags faststart  -f mp4')
    end
  end
end
